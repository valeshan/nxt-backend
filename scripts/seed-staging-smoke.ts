...
/**
 * Seeds a minimal, low-risk dataset for staging smoke tests:
 * - Test user with known credentials
 * - Organisation + Location
 * - Supplier + Product
 * - One Xero invoice with a line item tied to the supplier/product
 *
 * Guards to avoid misuse:
 * - Requires ENVIRONMENT_NAME=staging (or ALLOW_STAGING_SMOKE_SEED=true override)
 * - Safe to re-run (idempotent via find-or-create pattern)
 */
async function main() {
  ...
  console.log('[seed] Starting staging smoke seed...');
  ...
  if (!user) {
    ...
    // Ensure password is the expected one to make the smoke login deterministic
